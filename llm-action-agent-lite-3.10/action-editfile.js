(() => {
  ...
  async function getFileHandle(ctx, subPath, create) {
    ...
    if (!handle) throw new Error('未授权文件夹');
  ...
  async function E(A, C) {
    ...
    switch (o) {
      ...
      case 'replaceLines': {
        ...
        else if (A.pattern != null) {
          let idx = findExactLineIndex(lines, A.pattern);
          if (idx === -1) idx = findLineIndex(lines, A.pattern);
          if (idx === -1) throw new Error('未找到匹配的行: ' + A.pattern);
          from = idx; to = A.to != null ? A.to : from + 1;
        } else throw new Error('需要 from+to 或 pattern');
        if (from < 0 || to > lines.length || from >= to) throw new Error('无效的行范围');
        const replacementLines = (A.replacement || '').split('\n');
        lines.splice(from, to - from, ...replacementLines);
        c = lines.join('\n');
        break;
      }
      case 'insertLines': {
        ...
        if (A.at != null) at = A.at;
          ...
          const where = A.where || 'before';
          let idx = findExactLineIndex(lines, A.pattern);
          if (idx === -1) idx = findLineIndex(lines, A.pattern);
          if (idx === -1) throw new Error('未找到匹配的行: ' + A.pattern);
          at = where === 'after' ? idx + 1 : idx;
        } else throw new Error('需要 at 或 pattern');
        if (at < 0 || at > lines.length) throw new Error('at 超出范围');
        const insertContent = getContent(A);
        if (insertContent == null) throw new Error('insertLines 需要 text 或 content');
    ...
    try {
      const actionFile = window.AgentActionFile;
      if (actionFile && actionFile.refreshContextFileIfNeeded) {
        const rootHandle = await C.getBrowserHandleForWorkspace(ws);
        if (rootHandle) {
          contextRefreshed = await actionFile.refreshContextFileIfNeeded(p, C, rootHandle);
        }
      }
    } catch (e) {
      console.warn('edit_file 自动刷新上下文池失败:', p, e.message);
    }
    return {
      path: p, op: o,
      lengthBefore: r.content.length, lengthAfter: c.length,
      changed: r.content !== c,
      contextRefreshed
    };
  }
