(() => {
  ...
  function normalizeUsage(raw) {
    ...
    return { promptTokens, completionTokens, reasoningTokens: reasoning, totalTokens, estimated: false };
  }
  ...
  function renderUsageHtml(usage) {
    if (!usage) return '';
    return '<div class="' + (usage.estimated ? 'message-usage estimated' : 'message-usage') + '">' + core.escapeHtml(formatUsageLabel(usage)) + '</div>';
  }
  ...
  function buildFeedbackMessage(actionResults) {
    var parts = actionResults.map(function(r) {
      ...
      var resultStr;
      if (r.action.type === 'read_file' && r.result && r.result.content != null) resultStr = r.result.content;
      else resultStr = JSON.stringify(r.result, null, 2);
