(() => {
  ...
  function appendMessage(role, content, actionResults, meta) {
    ...
    if (role === 'assistant') {
      ...
      bubbleContent = renderReasoningHtml(m.reasoning, false) + '<div class="message-bubble">' + bubbleContent + '</div>' + (m.usage ? executor.renderUsageHtml(m.usage) : '');
    } else {
  ...
  function renderChatMessages() {
    ...
    for (const m of msgs) { if (m.hidden) continue; appendMessage(m.role, m.content, m.actionResults, { fileBlocks: m.fileBlocks, reasoning: m.reasoning, usage: m.usage }); }
    _scrollDown(true);
  }
  ...
  async function continueConversation() {
    ...
    try {
      ...
      const actions = fmt.parseActions(reply);
  ...
  async function maybeAutoNameConversation() {
    if (!core.getConfig().autoNameConversation) return;
    const conv = core.getActiveConversation();
    if (!conv || conv.titleAuto || conv.title !== '新对话') return;
    const msgs = core.getMessages().filter(m => m.role === 'user' && !(m.content || '').startsWith('以下是动作执行结果'));
    if (msgs.length < 1 || !core.getMessages().some(m => m.role === 'assistant')) return;
    try { await generateConversationTitle(); } catch {}
  }

  async function generateConversationTitle() {
    const conv = core.getActiveConversation();
    if (!conv) return;
    if (!core.getConfig().apiKey) { console.warn('智能命名: 无 API Key'); return; }
    const msgs = core.getMessages().filter(m => m.role === 'user' || m.role === 'assistant').slice(0, 8);
    const snippet = msgs.map(m => m.role + ': ' + (m.content || '').slice(0, 300)).join('\n');
    if (!snippet.trim()) { console.warn('智能命名: 对话内容太少'); return; }
    try {
      const title = await llm.callLLMForTitle(snippet);
      if (title) { conv.title = title.slice(0, 40); conv.titleAuto = true; core.saveConversationsStore(); updateChatTitle(); if (window.__agentPanels?.renderConversationList) window.__agentPanels.renderConversationList(); }
    } catch (err) { console.warn('智能命名失败:', err.message); }
  }
  ...
  window.__agentChatUI = {
    ...
    generateConversationTitle, maybeAutoNameConversation,
